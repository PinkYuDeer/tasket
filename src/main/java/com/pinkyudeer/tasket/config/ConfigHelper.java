package com.pinkyudeer.tasket.config;
...
import com.pinkyudeer.tasket.core.ConfigSetting;
...
public class ConfigHelper {
    ...
    public static void synchronizeConfiguration(boolean loadFromFile) {
        try {
            ...
            Tasket.LOG.error("配置文件加载失败，使用默认值", e);
        } finally {
    ...
    private static <T> T getConfigValue(String configKey, Class<T> type) {
        ...
        if (entry == null) {
            Tasket.LOG.error("配置项不存在: {}", configKey);
            return null;
        }
